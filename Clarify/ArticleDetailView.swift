...
struct ArticleDetailView: View {
    ...
    @State private var isPencilExpanded = false
    @State private var isBookExpanded = false
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                Text(article.title)
                    .font(.system(size: 30.4, weight: .bold, design: .default))
                    .padding(.bottom, 10)
                ...
                ForEach(article.content) { contentItem in
                    ...
                    case .heading(let text, let level):
                        Text(text)
                            .font(.system(size: headingSize(for: level) * 0.95, weight: .bold, design: .default))
                            .padding(.top, level == 1 ? 20 : 16)
                            .padding(.bottom, 8)
        ...
        // Floating Reading Comprehension Buttons
        HStack(alignment: .bottom) {
            // Pencil Button (Bottom Left) - Annotation Drawer
            Button(action: {
                withAnimation(.spring(response: 0.4, dampingFraction: 0.8)) {
                    isPencilExpanded.toggle()
                }
            }) {
                VStack(spacing: isPencilExpanded ? 15 : 0) {
                    // Top annotation tools (fade in when expanded)
                    if isPencilExpanded {
                        VStack(spacing: 20) {
                            Button(action: {}) {
                                Text("B")
